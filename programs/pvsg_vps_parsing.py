...
# get video list
anno_df_pass = load_file(f"./{data_dir}/PVSG.csv", "csv")
...
save_file(pass_map, f"./{data_dir}/palette_map.json", "json")

# metadata
metadata = dict()
image_root = f"./{data_dir}/images"
for vid in pass_vids:
    metadata[vid] = get_video_metadata(vid, image_root)
save_file(metadata, f"./{data_dir}/metadata.json", "json")
...
save_file(object_anno_map, f"./{data_dir}/obj_anno_map.json", "json")


# merging all info
def get_objects_info_vid(palette_map_vid):
    objects = []
...
for vid in pass_vids:
    ...
    pvsg_objects[vid] = get_objects_info_vid(palette_map_vid)
save_file(pvsg_objects, f"./{data_dir}/pvsg_objects.json", "json")
...
save_file(pvsg_full, f"./{data_dir}/pvsg.json", "json")
